...
​    创建一个导出类的对象时，该对象包含了一个基类的子对象。这个子对象与你用基类直接创建的对象是一样的。二者的区别在于，后者来自外部，而基类的子对象被包装导出类对象内部。
...
super 关键字用来访问父类内容，而this关键字用来访问本类内容  
  ...
  * super()和this两个构造调用，不能同时使用  
  ...
  抽象类，抽象方法所在的类，必须是 抽象类才行，在class之前写上abstract
   ...
   * 如何使用抽象类和抽象方法
   ...
   一个抽象类不一定含有抽象方法，只要保证抽象方法所在的类是 抽象类
